...
const Main = () => {
  ...
  const s3Credential: Credential<S3CredentialJson> | undefined =
    credentialsData.find(
      (credential) => credential.credential_json?.profile_name
    );
  ...
  return (
    <>
      ...
      {s3Credential ? (
        <>
          ...
          <div className="flex mb-1 text-sm">
            ...
            <p className="ml-1 italic my-auto">
              {s3Credential.credential_json.profile_name}
            </p>{" "}
            <button
        ...
        <>
          <Text>
            <ul className="list-disc mt-2 ml-4">
              <li>If AWS Access Key ID and AWS Secret Access Key are provided, they will be used for authenticating the connector.</li>
              <li>Otherwise, the Profile Name will be used (if provided).</li>
              <li>If no credentials are provided, then the connector will try to authenticate with any default AWS credentials available.</li>
            </ul>
          </Text>
          <Card className="mt-4">
      ...
      <Title className="mb-2 mt-6 ml-auto mr-auto">
        Step 2: Which bucket do you want to make searchable?
      </Title>
      ...
      {s3Credential && (
        <>
          <Card className="mt-4">
            ...
            <Text className="mb-4">
              Press connect below to start the connection to your S3
              bucket.
            </Text>
            <ConnectorForm<S3Config>
              ...
              formBodyBuilder={(values) => (
                <div>
                  ...
                  <TextFormField name="prefix" label="Path Prefix (optional):" />
                </div>
              )}
              validationSchema={Yup.object().shape({
...
export default function Page() {
  return (
    <div className="mx-auto container">
      ...
      <AdminPageTitle icon={<S3Icon size={32} />} title="S3" />
      <Main />
    </div>
  );
}
