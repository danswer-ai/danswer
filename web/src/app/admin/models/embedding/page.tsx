...
//     useState<FullCloudbasedEmbeddingModelDescriptor | null>(null);


//   const [tentativeNewEmbeddingModel, setTentativeNewEmbeddingModel] =
//     useState<FullEmbeddingModelDescriptor | null>(null);
...
//   const onSelect = async (model: FullEmbeddingModelDescriptor) => {
...
//   const onConfirm = async (model: FullEmbeddingModelDescriptor) => {
//     const response = await fetch(
//       "/api/secondary-index/set-new-embedding-model",
//       {
//         method: "POST",
...
//                 <div className="gap-4 mt-2 pb-10 flex content-start flex-wrap">
//                   {AVAILABLE_CLOUD_MODELS.map((provider, ind) => {
//                     const providerModels = AVAILABLE_CLOUD_MODELS.filter(model => model.id === provider.id);
...
//                             <div
//                               key={index}
//                               className={`p-3 mb-2 border-2  border-neutral-300 border-opacity-40 rounded-md rounded cursor-pointer ${provider.configured ? selectedModel.name == model.name ? 'bg-teal-50 border border-blue-300' : 'hover:bg-blue-50' : 'hover:bg-rose-50'
//                                 }`}
//                               onClick={() => {
//                                 if (model.name == "") {
//                                   return
...
//                               {/* Dimensions: {model.model_names[0]} | Price: ${model?.toFixed(3) ?? 'N/A'} per million tokens */}
//                               {/* </div> */}
//                             </div>
//                           ))}
//                         </div>
