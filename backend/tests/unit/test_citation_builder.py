...
from danswer.chat.models import CitationInfo, DanswerAnswerPiece, LlmDoc
...
@pytest.mark.parametrize("test_name, input_tokens, expected_text, expected_citations", [
    (
        "Single citation",
        ["Gro", "wth! [", "1", "]", "."],
...
def test_citation_extraction(mock_data, test_name, input_tokens, expected_text, expected_citations):
    final_answer_text, citations = process_text(input_tokens, mock_data)
    assert final_answer_text.strip() == expected_text.strip(), f"Test '{test_name}' failed: Final answer text does not match expected output."
    assert [citation.document_id for citation in citations] == expected_citations, f"Test '{test_name}' failed: Citations do not match expected output."
