...
def check_for_vespa_sync_task() -> None:
    ...
    try:
        ...
        with Session(get_sqlalchemy_engine()) as db_session:
            ...
            # check if any user teamspaces are not synced
            try:
...
def try_generate_stale_document_sync_tasks(
    ...
    # but I feel it's more understandable to teamspace the docs by cc_pair
...
def try_generate_document_set_sync_tasks(
    ...
    # It's possible for sets/teamspaces to be generated initially with no entries
        ...
        with Session(get_sqlalchemy_engine()) as db_session:
            ...
            for key_bytes in r.scan_iter(RedisTeamspace.FENCE_PREFIX + "*"):
                monitor_teamspace_taskset = (
                    fetch_versioned_implementation_with_fallback(
                        "enmedd.background.celery.tasks.vespa.tasks",
                        "monitor_teamspace_taskset",
                        noop_fallback,
                    )
                )
                monitor_teamspace_taskset(key_bytes, r, db_session)
