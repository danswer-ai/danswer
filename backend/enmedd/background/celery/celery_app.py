...
from enmedd.background.celery.celery_redis import RedisConnectorCredentialPair
from enmedd.background.celery.celery_redis import RedisConnectorDeletion
from enmedd.background.celery.celery_redis import RedisDocumentSet
from enmedd.background.celery.celery_redis import RedisTeamspace
from enmedd.background.celery.celery_utils import extract_ids_from_runnable_connector
from enmedd.background.celery.celery_utils import should_prune_cc_pair
from enmedd.background.connector_deletion import delete_connector_credential_pair_batch
...
from enmedd.configs.constants import CELERY_VESPA_SYNC_BEAT_LOCK_TIMEOUT
from enmedd.configs.constants import EnmeddCeleryPriority
from enmedd.configs.constants import EnmeddRedisLocks
from enmedd.configs.constants import POSTGRES_CELERY_BEAT_APP_NAME
from enmedd.configs.constants import POSTGRES_CELERY_WORKER_APP_NAME
...
from enmedd.db.connector_credential_pair import add_deletion_failure_message
...
from enmedd.db.document import count_documents_by_needs_sync
...
from enmedd.db.document_set import delete_document_set
from enmedd.db.document_set import delete_document_set_cc_pair_relationship__no_commit
from enmedd.db.document_set import fetch_document_sets
from enmedd.db.document_set import fetch_document_sets_for_document
from enmedd.db.document_set import get_document_set_by_id
from enmedd.db.document_set import mark_document_set_as_synced
...
from enmedd.redis.redis_pool import RedisPool
from enmedd.server.documents.models import ConnectorCredentialPairIdentifier
from enmedd.utils.logger import ColoredFormatter
from enmedd.utils.logger import PlainFormatter
...
from enmedd.utils.variable_functionality import noop_fallback
...
def check_for_vespa_sync_task() -> None:
    ...
    lock_beat = r.lock(
        EnmeddRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK,
...
def check_for_connector_deletion_task() -> None:
    ...
    lock_beat = r.lock(
        EnmeddRedisLocks.CHECK_CONNECTOR_DELETION_BEAT_LOCK,
        timeout=CELERY_VESPA_SYNC_BEAT_LOCK_TIMEOUT,
    )
...
def monitor_vespa_sync() -> None:
    ...
    lock_beat = r.lock(
        EnmeddRedisLocks.MONITOR_VESPA_SYNC_BEAT_LOCK,
        timeout=CELERY_VESPA_SYNC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        # length = celery_get_queue_length(EnmeddCeleryQueues.VESPA_METADATA_SYNC, r_celery)
        # task_logger.warning(f"queue={EnmeddCeleryQueues.VESPA_METADATA_SYNC} length={length}")
    except SoftTimeLimitExceeded:
        task_logger.info(
            "Soft time limit exceeded, task is being terminated gracefully."
        )
...
def on_worker_init(sender: Any, **kwargs: Any) -> None:
    ...
    r.delete(EnmeddRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK)
    r.delete(EnmeddRedisLocks.MONITOR_VESPA_SYNC_BEAT_LOCK)
    ...
    for key in r.scan_iter(RedisTeamspace.TASKSET_PREFIX + "*"):
        r.delete(key)

    for key in r.scan_iter(RedisTeamspace.FENCE_PREFIX + "*"):
        r.delete(key)
...
celery_app.conf.beat_schedule = {
    "check-for-vespa-sync": {
        ...
        "options": {"priority": EnmeddCeleryPriority.HIGH},
    },
}
...
celery_app.conf.beat_schedule.update(
    {
        "kombu-message-cleanup": {
            ...
            "options": {"priority": EnmeddCeleryPriority.LOWEST},
        },
    }
)
celery_app.conf.beat_schedule.update(
