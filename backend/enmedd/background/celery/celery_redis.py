...
from enmedd.background.celery.celeryconfig import CELERY_SEPARATOR
...
from enmedd.configs.constants import EnmeddCeleryQueues
from enmedd.db.connector_credential_pair import get_connector_credential_pair_from_id
from enmedd.db.document import construct_document_select_for_connector_credential_pair
from enmedd.db.document import (
    construct_document_select_for_connector_credential_pair_by_needs_sync,
)
from enmedd.db.document_set import construct_document_select_by_docset
    ...
    def generate_tasks(
        ...
        for doc in db_session.scalars(stmt).yield_per(1):
            ...
            result = celery_app.send_task(
                ...
                queue=EnmeddCeleryQueues.VESPA_METADATA_SYNC,
                task_id=custom_task_id,
                priority=EnmeddCeleryPriority.LOW,
            )
...
class RedisTeamspace(RedisObjectHelper):
    PREFIX = "teamspace"
    FENCE_PREFIX = PREFIX + "_fence"
    TASKSET_PREFIX = PREFIX + "_taskset"
    ...
    def generate_tasks(
        ...
        try:
            construct_document_select_by_teamspace = fetch_versioned_implementation(
                "enmedd.db.teamspace",
                "construct_document_select_by_teamspace",
            )
        except ModuleNotFoundError:
            return 0

        stmt = construct_document_select_by_teamspace(self._id)
        for doc in db_session.scalars(stmt).yield_per(1):
            current_time = time.monotonic()
            if current_time - last_lock_time >= (
                CELERY_VESPA_SYNC_BEAT_LOCK_TIMEOUT / 4
    ...
    def generate_tasks(
        ...
        for doc in db_session.scalars(stmt).yield_per(1):
            ...
            result = celery_app.send_task(
                ...
                queue=EnmeddCeleryQueues.VESPA_METADATA_SYNC,
                task_id=custom_task_id,
                priority=EnmeddCeleryPriority.MEDIUM,
            )
    ...
    def generate_tasks(
        ...
        for doc in db_session.scalars(stmt).yield_per(1):
            ...
            result = celery_app.send_task(
                ...
                queue=EnmeddCeleryQueues.CONNECTOR_DELETION,
                task_id=custom_task_id,
                priority=EnmeddCeleryPriority.MEDIUM,
            )
...
def celery_get_queue_length(queue: str, r: Redis) -> int:
    ...
    total_length = 0
    for i in range(len(EnmeddCeleryPriority)):
        queue_name = queue
        if i > 0:
