...
def get_danswer_api_key(key_len: int = 30, dont_regenerate: bool = True) -> str | None:
...
def api_key_dep_if_exist(api_key: str | None = Header(None)) -> str | None:
    saved_key = get_danswer_api_key(dont_regenerate=True)
    if not saved_key:
        return None
...
def document_ingestion(
    ...
    if (
        doc_info.public_doc
        and doc_info.credential_id is not None
        and doc_info.credential_id != 0
    ):
        raise ValueError("Invalid Credential for doc, conflicting values")

    if doc_info.public_doc:
        credential_id = 0
    elif doc_info.credential_id:
        ...
        credential = create_credential(
            CredentialBase(credential_json={}, admin_public=True),
            user=None,
            db_session=db_session,
        )
    ...
    if connector_id is not None:
        ...
        if connector is None:
            raise ValueError("Invalid Connector for doc, does not exist")
    else:
        connector_id = 0
    ...
    document.doc_updated_at = datetime.utcnow()
