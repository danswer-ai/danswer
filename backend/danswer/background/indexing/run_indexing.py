...
def get_document_generator(
    ...
    elif task == InputType.PRUNE:
        if isinstance(runnable_connector, PruneConnector):
            doc_batch_generator = runnable_connector.prune_connector()
...
def prepare_index_attempt(db_session: Session, index_attempt_id: int) -> IndexAttempt:
...
def run_indexing_entrypoint(index_attempt_id: int) -> None:
    ...
    try:
        ...
        with Session(get_sqlalchemy_engine()) as db_session:
            ...
            # as in progress
            attempt = prepare_index_attempt(db_session, index_attempt_id)
