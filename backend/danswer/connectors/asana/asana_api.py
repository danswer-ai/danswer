...
class AsanaTask:
    ...
    def __init__(self, id: str, title: str, text: str, link: str, last_modified: datetime,
                    project_gid: str, project_name: str) -> None:
        self.id = id
        self.title = title
        self.text = text
        self.link = link
...
class AsanaAPI:
    ...
    def get_tasks(self, project_gids: list[str] | None, start_date: str) -> Iterator[AsanaTask]:
        ...
        projects = self.project_api.get_projects(opts={
        ...
        for project_info in projects:
            ...
            if project_gids is None or project_gid in project_gids:
                ...
                logger.info(f"Skipping project: {project_gid} - not in accepted project_gids")
            project_count += 1
            if project_count % 100 == 0:
                logger.info(f"Found {project_count} projects so far...")
        logger.info(f"Found {len(projects_list)} projects, iterating over their tasks now...")
        for project_gid in projects_list:
            for task in self.get_tasks_for_project(project_gid, start_date, start_seconds):
                yield task
        logger.info(f"Done getting {self.task_count} docs")
        logger.info(f"Total API errors: {self.api_error_count}")

    def get_tasks_for_project(self, project_gid: str, start_date: str, start_seconds: int) -> Iterator[AsanaTask]:
        # Indexing logic:
        ...
        if not project['team'] or not project['team']['gid']:
            logger.info(f"Skipping project that does not have a team: {project['name']} ({project_gid})")
            return []
        if project['privacy_setting'] == 'private':
            ...
            if self.team_gid and project['team']['gid'] != self.team_gid:
                logger.info(f"Skipping private project that does not have the configured team id '{self.team_gid}': "
                      + f"{project['name']} ({project_gid}) (team: {project['team']['name']})")
                return []
            else:
                logger.info(f"Indexing private project that does have the configured team id '{self.team_gid}': "
                      + f"{project['name']} ({project_gid}) (team: {project['team']['name']})")
        simple_start_date = start_date.split(".")[0].split("+")[0]
        logger.info(f"Indexing tasks modified since {simple_start_date} of project {project['name']} ({project_gid})")
        opts = {
        ...
        for data in tasks_from_api:
            ...
            if self.task_count % 10 == 0:
                ...
                if runtime_seconds > 0:
                    logger.info(f"Got {self.task_count} tasks in {runtime_seconds:.0f} seconds = "
            ...
            if data['created_by'] and data['created_by']['gid']:
                text += "This issue was created by " + self.get_user(data['created_by']['gid'])['name']
                text += " on " + self.format_date(data['created_at']) + ".\n"
            if data['due_on']:
                text += "This issue is due on " + self.format_date(data['due_on']) + ".\n"
            if data['completed_at']:
                text += "This issue was completed on " + self.format_date(data['completed_at']) + ".\n"
            text += "\n"
            try:
                stories_opts: Dict[str, str] = {}
                story_start = time.time()
                stories = self.stories_api.get_stories_for_task(data['gid'], stories_opts)
                story_count = 0
                ...
                for story in stories:
                    ...
                    if story['resource_subtype'] == "comment_added":
                        ...
                        text += "Comment by " + by_user['name'] + ": " + comment['text'] + "\n\n"
                        story_comment_count += 1
                logger.info(f"Got {story_count} stories (with {story_comment_count} comments) in "
                            + f"{time.time() - story_start:.2f} seconds")
                text += "This issue was last modified on " + self.format_date(data['modified_at']) + ".\n"
                last_modified = datetime.fromisoformat(data['modified_at'])
                task = AsanaTask(id=data['gid'], title=data['name'], text=text,
                                link=data['permalink_url'], last_modified=last_modified,
                                project_gid= project_gid, project_name=project['name'])
                yield task
            except Exception:
                logger.error(f"Error getting stories for task {data['gid']} in project {project_gid} "
