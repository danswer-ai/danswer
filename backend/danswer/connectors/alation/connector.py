...
class AlationClientNotSetUpError(PermissionError):
    def __init__(self) -> None:
        super().__init__("Alation Client is not set up, was load_credentials called?")
...
class AlationClient:
    ...
    def create_url(self, url: str, params: Optional[dict[str, str]] = None) -> str:
        """Helper for forming Alation URLs"""
        if params is not None:
            return f"{self.base_url}{url}?{urlencode(params)}"
        return f"{self.base_url}{url}"

    def _create_url_for_object(
        self, otype: AlationObjectType, params: Optional[dict[str, str]] = None
    ) -> str:
    ...
    def authenticate(self) -> None:
        ...
        response = self.api_client.post(
            ...
            json={"user_id": self.user_id, "refresh_token": self.refresh_token},
        )
        data = response.json()
        self.api_client.headers.update({"Token": data["api_access_token"]})
    ...
    def _validate_access_token(self) -> None:
        ...
        if (
            ...
            or datetime.now(self.token_expiry.tzinfo) >= self.token_expiry
        ):
            self.authenticate()

    def get_batch_for_object_type(
        self, otype: AlationObjectType, limit: int = 100, skip: int = 0
    ) -> list[dict[str, Any]]:
        ...
        response = self.api_client.get(
            self._create_url_for_object(
                otype, params={"limit": str(limit), "skip": str(skip)}
            ),
        )
...
class AlationConnector(LoadConnector):
    ...
    def _get_batch_for_object_type(
        self, otype: AlationObjectType, start_index: int
    ) -> tuple[list[Document], int]:
        doc_batch: list[Document] = []

        try:
            batch = self.client.get_batch_for_object_type(
                otype, limit=self.batch_size, skip=start_index
            )
    ...
    def load_from_state(self) -> GenerateDocumentsOutput:
        ...
        for otype in self.object_types_to_index:
            ...
            start_index = 0
            while True:
                doc_batch, num_pages = self._get_batch_for_object_type(
                    otype, start_index
                )
                ...
                if (
                    ...
                    or start_index >= self.max_objects_to_index_per_type
                ):
                    break
