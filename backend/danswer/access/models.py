...
class DocumentAccess(ExternalAccess):
    ...
    def to_acl(self) -> set[str]:
        return set(
            ...
            + ([PUBLIC_DOC_PAT] if self.is_externally_public else [])
        )
    ...
    def build(
        ...
        return cls(
            external_user_emails={
                prefix_user_email(user_email) for user_email in external_user_emails
            },
            external_user_group_ids={
                prefix_external_group(group_name)
                for group_name in external_user_group_ids
            },
            user_emails={prefix_user_email(user_email) for user_email in user_emails},
            user_groups=set(user_groups),
            is_externally_public=is_externally_public,
        )
