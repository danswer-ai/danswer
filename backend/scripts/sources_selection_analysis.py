...
class SelectionAnalysis:
    ...
    def do_request(self, query: str) -> dict:
        """Request the Arnold AI API

        Args:
            query (str): A query

        Returns:
            dict: The Arnold AI API response content
        """
        ...
        try:
            ...
            if response.status_code != 200:
                color_output(
                    (
                        "something goes wrong while requesting the Arnold AI API "
            ...
            color_output(
                f"Unable to request the Arnold AI API for the query '{query}': {e}",
                model="critical",
            )
            sys.exit(1)
    ...
    def extract_content(self, contents: dict) -> dict:
        """Extract the content returns by the Arnold AI API

        Args:
            contents (dict): The enmedd response content
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Currently, to get this script working when the Arnold AI Auth is "
    ...
    parser.add_argument(
        ...
        default=3000,
        help=(
            "The Arnold AI Web (not the API) port. We use the UI to forward the requests to the API. "
            "It should be '3000' for local dev and '80' if Arnold AI runs using docker compose."
        ),
    )
    parser.add_argument(
        "-q" "--queries",
