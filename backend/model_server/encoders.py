...
class CloudEmbedding:
    ...
    def _embed_openai(self, texts: list[str], model: str | None) -> list[Embedding]:
        ...
        # the context lengths used by Arnold AI currently are smaller than the max token length
    ...
    def _embed_cohere(
        ...
        for text_batch in batch_list(texts, _COHERE_MAX_INPUT_LEN):
            # Does not use the same tokenizer as the Arnold AI API server but it's approximately the same
